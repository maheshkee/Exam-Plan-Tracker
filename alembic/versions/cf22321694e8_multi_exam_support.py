...
def upgrade() -> None:
    ...
    if bind.dialect.name == "sqlite":
        # SQLite: must rebuild table to remove unique constraint
        ...
        op.execute("""
            INSERT INTO user_exams_new (
                ...
                study_hours_per_day, is_active, created_at
            )
            SELECT
                id, user_id, exam_id, exam_date,
                study_hours_per_day, 1, created_at
            FROM user_exams
        """)
        ...
        # PostgreSQL: can add column and drop constraint directly
        op.add_column(
            "user_exams",
    ...
    if bind.dialect.name == "sqlite":
        ...
        op.execute("""
            INSERT INTO user_exams_old (
                id, user_id, exam_id, exam_date,
                study_hours_per_day, created_at
            )
            SELECT
                id, user_id, exam_id, exam_date,
                study_hours_per_day, created_at
        ...
        op.drop_column("user_exams", "is_active")
